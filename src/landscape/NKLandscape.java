...
public class NKLandscape implements FitnessFunction {
		...
		public NKLandscape(int seed) {
			...
			double[][] interactionTable = generateRandomInteractionTable(Constants.N, Constants.K);
			generateFitnessTable(interactionTable); //this has no return value because it stores its data in the landscape globals
		}
		
		/**
		 * The NKLandscape stores all of its fitnesses in the fitTable. In order
		 * to get the fitness of a NKPhenotype we need to convert the bitstring
		 * into an integer
		 * 
		 * @param p
		 * @return
		...
		public double getFitness(Phenotype p) {
			...
			//Get the fitness of the phenotype from our table using the provided tableIndex function
		...
		public void generateFitnessTable(double[][] interactionTable) {
			double[] fitnessTable = new double[1<<Constants.N];
			double minFitness = 100000;
			double maxFitness = 0;

			for (int genotypeInt = 0; genotypeInt < ((int) Math.pow(2, Constants.N)); genotypeInt++)
			{
				double fitness = 0;

				for (int gene = 0; gene < Constants.N; gene++) // loops through each gene (bit)
				{
					int interactions = 0;
					for (int neighbor = 0; neighbor < Constants.K + 1; neighbor++) // loop through all our neighbors
					{
						int neighborIndex = (gene + neighbor) % Constants.N;
						interactions = interactions|((genotypeInt>>>(Constants.N-1-neighborIndex))&1)<<(Constants.K-neighbor);
					}
